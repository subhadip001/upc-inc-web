...
const UserDetailsAdmin = ({ userDetails, setUserDetails }) => {
  const progress = useRef(0);
  const [x, setX] = useState(0);
  const [totalFiles, setTotalFiles] = useState(0);
  const [userNew, setEditteduser] = useState(userDetails);
  const deleteUser = async () => {
    try {
      await axios
        .delete("https://api.subhadipmandal.engineer/upc/api/v1/deleteOne", {
  ...
  return (
    <div className="user-details-sec">
      <h1>User Details : {userDetails.upc_id}</h1>
